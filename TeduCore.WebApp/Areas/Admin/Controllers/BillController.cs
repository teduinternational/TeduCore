    ...
    public class BillController : BaseController
    {
        ...
        public IActionResult ConfirmBill(Guid billId)
        {
            try
            {
                ...
                _billService.Save();
                return new OkObjectResult("Xác nhận đơn hàng thành công.");
            }
            catch (Exception e)
            {
        ...
        public IActionResult CancelBill(Guid billId)
        {
            try
            {
                ...
                _billService.Save();
                return new OkObjectResult("Đơn hàng đã được huỷ thành công.");
            }
            catch (Exception e)
            {
                return new BadRequestObjectResult(e.Message);
        ...
        public IActionResult PendingBill(Guid billId)
        {
            try
            {
                _billService.PendingBill(billId);
                _billService.Save();
                return new OkObjectResult("Đã cập nhật hoãn thành công.");
            }
            catch (Exception e)
            {
                return new BadRequestObjectResult(e.Message);
        ...
        public IActionResult ExportExcel(Guid billId)
        {
            ...
            using (FileStream templateDocumentStream = System.IO.File.OpenRead(templateDocument))
            {
                using (ExcelPackage package = new ExcelPackage(templateDocumentStream))
                {
                    ...
                    worksheet.Cells[6, 1].Value = "Điện thoại: " + billDetail.CustomerMobile;
